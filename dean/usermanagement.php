...
// Fetch the club ID of the logged-in dean
function getDeanClub()
{
    global $conn;
    $deandId = $_SESSION['user_id'];
...
$deanClubId = getDeanClub();
if (!$deanClubId) {
    header('Content-Type: application/json');
    echo json_encode(['error' => 'No club assigned to this dean']);
    exit();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action'])) {
        switch ($_POST['action']) {
            case 'fetchUsers':
                handleFetchUsers($deanClubId);
                break;
            case 'addUser':
                handleAddUser($deanClubId);
                break;
            case 'editUser':
                handleEditUser($deanClubId);
                break;
            case 'deleteUser':
                handleDeleteUser($deanClubId);
                break;
            default:
...
<head>
    ...
    <title>Dean - User Management</title>
